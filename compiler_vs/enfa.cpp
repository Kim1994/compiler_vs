...
vector<enfanode*> or(NodeTree *nodetree, vector<enfanode*> N1, vector<enfanode*> N2){
	...
	s->out.push_back(lin1);
	s->out.push_back(lin2);
	s->FinalStatus = 0;
	lin1->start = s;
	lin1->end = N1[0];
	lin1->match = "";
	...
	N1[1]->FinalStatus = 0;
	N2[1]->FinalStatus = 0;
	N1[0]->in.push_back(lin1);
	N2[0]->in.push_back(lin2);
	N1[1]->out.push_back(lout1);
	N2[1]->out.push_back(lout2);
	vector<enfanode*> re;
	re.push_back(s);
	re.push_back(e);
	return re;
...
vector<enfanode*> loop(NodeTree *nodetree, vector<enfanode*> N1){
	...
	lin->end = N1[0];
	lin->match = "";
	N1[0]->in.push_back(lin);
	lout->start = N1[1];
	lout->end = s;
	lout->match = "";
	N1[1]->out.push_back(lout);
	N1[1]->FinalStatus = 0;
	vector<enfanode*> re;
	re.push_back(s);
	re.push_back(s);
